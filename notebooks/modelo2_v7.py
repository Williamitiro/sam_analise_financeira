# MÓDULO: PÁGINA 2 - GROWTH MACHINE (V7.0 - GOLD STANDARD)
# ============================================================================
# DATA: 2025-12-06
...
def clean_html_for_markdown(text):
    ...
    # <span class='status-red'>TEXTO</span> -> **TEXTO**
    # Regex para capturar conteúdo dentro do span
...
def render_atomic_block(chart_id, title_colloquial, title_technical, fig, legend_md, df_tabela, insight_dict, formulas_md=None, report_mode=False):
    ...
    """
    # NOVO: Quebra de página ANTES do bloco se estiver em modo relatório
    if report_mode:
        display(Markdown("\\newpage"))
...
def render_atomic_block(chart_id, title_colloquial, title_technical, fig, legend_md, df_tabela, insight_dict, formulas_md=None, report_mode=False):
    ...
    # 1. BLOCO A: TÍTULOS
    if report_mode:
        # Adiciona {.unnumbered .unlisted} para não poluir o Sumário (TOC)
    ...
    # 2. BLOCO B: GRÁFICO
    # Ajuste de tamanho para PDF (A4) - Reduzido para 6.0 para segurança total contra overflow
    if report_mode:
        fig.set_size_inches(6.0, 3.2) 
        # Tenta centralizar (no Quarto, o display(fig) padrão alinha a esquerda as vezes)
    ...
    if report_mode:
        # Markdown table limpa e COMPACTA
        df_clean = df_tabela.copy()
        
        # Encurtar colunas para caber na largura
        new_cols = []
    ...
    if formulas_md:
        if report_mode:
            # Limpa HTML da auditoria também
            audit_clean = clean_html_for_markdown(formulas_md)
            
            # Quarto Callout normal (Sem Minipage para evitar erro de parser Pandoc)
            audit_md = f"""
